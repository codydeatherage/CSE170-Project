...
void GlrMyNode::render(SnShape* s, GlContext* ctx)
{
	...
	if (s->changed() & SnShape::Changed) // flags are: Unchanged, RenderModeChanged, MaterialChanged, Changed
	{
		...
		float inc = (float)(2 * M_PI / c.n); //equal division of circle
		...
		for (theta = 0.0f; theta < 2 * M_PI; theta += inc) {
			for (phi = 0.0f; phi < 2 * M_PI; phi += inc) {
				/*P1 (theta, phi)*/
				p1 = GsVec((R + r * cosf(theta)) * cosf(phi), (R + r * cosf(theta)) * sinf(phi), r * sinf(theta));

				/*P2 (theta + inc, phi)*/
				p2 = GsVec((R + r * cosf(theta + inc)) * cosf(phi), (R + r * cosf(theta + inc)) * sinf(phi), r * sinf(theta + inc));

				/*P3 (theta + inc, phi + inc)*/
				p3 = GsVec((R + r * cosf(theta + inc)) * cosf(phi + inc), (R + r * cosf(theta + inc)) * sinf(phi + inc), r * sinf(theta + inc));

				/*P1 again*/
				//GsVec p1 = GsVec((R + r * cosf(theta)) * cosf(phi), (R + r * cosf(theta)) * sinf(phi), r * sinf(theta));

				/*P3 again*/
				//GsVec p3 = GsVec((R + r * cosf(theta + inc)) * cosf(phi + inc), (R + r * cosf(theta + inc)) * sinf(phi + inc), r * sinf(theta + inc));

				/*P4 (theta, phi + inc)*/
				p4 = GsVec((R + r * cosf(theta)) * cosf(phi + inc), (R + r * cosf(theta)) * sinf(phi + inc), r * sinf(theta));
